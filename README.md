...
- 订单汇总：订单列表按当前筛选展示订单数、商品数和已支付金额。
- 订单同步：订单列表合并后端订单和本地 fallback 订单，避免数据丢失。
- 数据一致性：统一合并订单本地元数据，保留支付方式和评价等本地补充字段。
- 订单评价：已完成订单可提交本地评价，并在订单列表和订单详情中展示评价状态。
- 商家收藏：小程序支持本地收藏商家，个人中心显示收藏数量并可快速进入。
- 支付方式：个人中心支持本地切换模拟支付方式，确认订单和订单详情展示当前支付方式。
- 用户设置：个人中心支持订单通知、自动定位和开发信息显示等本地偏好。
